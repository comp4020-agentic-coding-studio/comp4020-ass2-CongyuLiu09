...
<div class="dash-flow">
  <div class="dash-flow__step"><span class="dash-flow__step-label">1. Symbol designed</span>A manufacturer draws its own symbol first</div>
  <div class="dash-flow__step"><span class="dash-flow__step-label">2. Function mandated</span>A regulator requires the function, not the icon</div>
  <div class="dash-flow__step"><span class="dash-flow__step-label">3. Symbols compared</span>Competing designs surveyed across manufacturers</div>
  <div class="dash-flow__step"><span class="dash-flow__step-label">4. Standard drafted</span>ISO/SAE selects or drafts one shared symbol</div>
  <div class="dash-flow__step"><span class="dash-flow__step-label">5. Standard propagates</span>Manufacturers redesign clusters to comply</div>
  <div class="dash-flow__step"><span class="dash-flow__step-label">6. Drivers learn it</span>Repetition across cars is what makes it "shared"</div>
</div>

```comment
Same six stages as the numbered list two slides back, redrawn as a chain —
useful to point at physically when a student asks "so when exactly does an
icon become standard," since the answer is always "step 6, never earlier."
```
...
<div class="dash-panel">
  <span class="dash-panel__label">Icons, against the six-question framework</span>

  **Urgency** the icon cannot express by itself — it names *which* fault,
  not *how bad*. Typically **foveal**, since a driver has to look at and
  identify the shape. **Meaning** entirely learned, never self-evident.
  Purely visual **modality**. **Timing** persistent for as long as the
  fault holds. **Required action** under-specified — "check it soon" and
  "pull over now" can trigger the identical amber pictogram.
</div>

---

## Common misconception: "a good icon should be self-explanatory"

It shouldn't have to be, and mostly isn't. No pictogram is self-explanatory
to a genuinely first-time viewer — the ISO tyre icon reads as obvious now
only because it has been seen dozens of times, on multiple cars, across
years. A tyre cross-section with an exclamation mark is not intrinsically
graspable; it is a learned shorthand that happens to have converged
industry-wide, which is a completely different achievement from being
inherently clear.

The tell: go back to the opening rental-car example. The icon in that
scenario was, by hypothesis, a plausible variant a driver had genuinely
never seen. Nothing about its shape alone resolved the ambiguity — only
prior exposure would have.

```notes
Push back if a student claims "well-designed" icons don't need learning at
all — ask them to draw, cold, an icon for a function they've genuinely
never encountered (tyre-sealant-level-low is a good one) and watch the
"self-explanatory" claim collapse in real time.
```
...
Two answer-shapes tend to cover most pairs' results:

1. **Icons you both got right, confidently** — almost always the heavily
   regulated ones (ISO tyre pressure, fuel pump). The real explanation is
   repeated exposure across many cars over a decade-plus of standardisation,
   not that the shape is obvious in isolation.
2. **Icons that produced disagreement** — usually generic "!" triangles and
   battery/charge symbols, where several genuinely different faults share a
   visually similar shape and no single regulatory push ever forced one
   drawing to win.

The underlying constraint the exercise is really testing: recognition speed
tracks *regulatory age*, not perceived cleverness of the drawing. A harder
question worth sitting with afterwards — if a genuinely new function (say,
an EV battery thermal-runaway warning) needs an icon today, with no TREAD
Act equivalent forcing convergence, what's the fastest realistic path to it
becoming shared rather than merely used?

```notes
If time allows, take a show of hands on the harder question rather than
answering it — there isn't a clean answer, and that's the point: absent a
regulatory trigger, convergence has historically taken a manufacturer
consortium, a voluntary ISO working group, or years of organic copying, all
slower than a mandate.
```
