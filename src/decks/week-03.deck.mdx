...
> **Suggested image:** A 1960s American full-size sedan dashboard with a
> factory-fitted ammeter or oil-pressure gauge, next to a same-model-year
> dashboard from a trim level that replaced it with a warning light
> **Notice:** the empty circular bezel or blanking plate where the gauge
> used to sit, and how little the rest of the cluster layout had to change
> to accommodate the swap
> **Annotation:** an arrow from the gauge's numbered scale to the light's
> single lens, labelled "80 psi of range → 1 bit"
> **Caption:** "Same model, same year, two different answers to the same
> question."
> **Alt text:** "Side-by-side comparison of two dashboards from the same
> car model and year: the left cluster has a round analogue gauge with a
> numbered scale for oil pressure, and the right cluster has a small round
> warning lamp in the same position, unlit."
...
> **Suggested image:** A modern production car's driver display with the
> check-engine (MIL) icon illuminated, driver's-eye view, dashboard otherwise
> unremarkable
> **Notice:** how little else on the cluster changes when this light comes
> on — no accompanying text, no severity indicator, no colour distinct from
> other amber telltales nearby
> **Annotation:** a circle around the MIL icon with a callout reading
> "1 of 400+ possible stored codes"
> **Caption:** "The dashboard's entire message is: something is wrong.
> Somewhere."
> **Alt text:** "A driver's-eye photograph of a car dashboard at night,
> showing an illuminated amber engine-shaped warning icon among several
> other unlit telltales, with no accompanying text or numeric readout."
...
> **Suggested image:** An OBD-II handheld scan tool plugged into a vehicle's
> diagnostic port, its small screen showing a specific stored code (e.g.
> "P0455 — Evaporative Emission System Leak Detected")
> **Notice:** how much more specific the scan tool's readout is than the
> dashboard light that sent the driver looking for it in the first place
> **Annotation:** a bracket connecting the dashboard's single lit icon (in
> a small inset) to the scan tool's specific code, labelled "same event, two
> very different amounts of information"
> **Caption:** "Everything the dashboard didn't tell you was sitting in the
> car the whole time."
> **Alt text:** "A handheld OBD-II diagnostic scan tool connected by cable
> to a car's dashboard-mounted diagnostic port, its screen displaying a
> specific alphanumeric fault code and a short description of the fault."
