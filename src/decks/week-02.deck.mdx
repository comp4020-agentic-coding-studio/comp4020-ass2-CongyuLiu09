...
<div class="dash-flow">
  <div class="dash-flow__step">
    <span class="dash-flow__step-label">1. Locate</span>
    Eye finds the gauge's position on the dash
  </div>
  <div class="dash-flow__step">
    <span class="dash-flow__step-label">2. Perceive</span>
    Needle angle registers
  </div>
  <div class="dash-flow__step">
    <span class="dash-flow__step-label">3. Recall</span>
    Angle compared to a remembered "normal" zone
  </div>
  <div class="dash-flow__step">
    <span class="dash-flow__step-label">4. Judge</span>
    Deviation from normal is assessed
  </div>
  <div class="dash-flow__step">
    <span class="dash-flow__step-label">5. Decide</span>
    Act now, soon, or not at all
  </div>
</div>

Stages 3 and 4 are where an unfamiliar car breaks the chain — there is
nothing printed on the dial that substitutes for the memory those two stages
assume.

```notes
Point at the gap between stage 2 and stage 3 specifically — that's the join
between what the dial actually prints and what the driver has to supply
from memory, and it's the join a rental-car driver never gets to build.
```
...
<div class="dash-panel">
  <span class="dash-panel__label">The continuous needle, against last week's six questions</span>

  - **Urgency** — low to medium for most of a drive; a needle rarely spikes urgency on its own
  - **Visibility** — foveal only — never used for anything peripheral
  - **Meaning** — has to be learned; nothing on the dial marks "normal"
  - **Modality** — visual only
  - **Timing** — continuous, not a single event
  - **Required driver action** — usually "nothing yet" — the needle's value is letting a trend be noticed before any one reading would demand action
</div>

```notes
This is the same six-question panel from week 1's fuel-gauge example, run
against this week's channel instead. Keep the format identical week to
week — the point students should notice is the *repetition*, not novelty.
```

---

## Common misconception: "a bigger dial is just a styling choice"

It's easy to read Porsche centring the tachometer, or a Camry centring the
speedometer, as brand aesthetics — sportscar versus family car, nothing more
substantive than badge design.

That undersells what a centred dial actually costs the driver: the biggest,
most central position is also the fastest, least-effortful glance on the
whole dashboard. Assigning it to one instrument is a permanent decision about
which number this car considers most urgent, made once at the factory and
then never revisited for the life of the car (unlike a reconfigurable
digital cluster, week 8 onward). Two manufacturers who made that decision
differently weren't expressing taste — they were making incompatible claims
about what their driver needs fastest.

```notes
Call back to the "side by side: two layouts, two claims" table a few slides
earlier — this misconception slide is arguing that the table isn't
describing two *styles*, it's describing two *design commitments* with real
downstream consequences (which glance a driver reaches for first, under
pressure).
```

---

## Exercise: argue for a re-centred cluster

Pick a vehicle type that isn't a sports car or a mainstream sedan — a
delivery van, a long-haul truck, a car intended mainly for learner drivers,
or an electric city car with no engine to rev.

In groups, decide:

1. Which single instrument should be centred and largest for *this*
   driver's actual priorities, and why?
2. What existing convention (Porsche's tach-centred, or the mainstream
   speedo-centred layout) does your answer resemble, if either?
3. Would your chosen layout survive the trained-reader problem — could a
   first-time driver of this vehicle read your centred instrument's
   "normal" zone without prior experience?

```notes
Give groups 4–5 minutes. Electric city cars are the interesting edge case —
no tachometer to centre at all, which forces groups to name a genuinely new
candidate (a power/regen gauge, a range estimate) rather than just picking
between the two conventions already on the table.
```
...
- **Delivery van / long-haul truck** — centring a load or engine-temperature
  gauge over the tachometer, since the driver already manages a much wider,
  more forgiving rev range than a sports car and the more urgent number is
  the vehicle's own mechanical health over a long shift
- **Electric city car** — centring a power/regeneration gauge in the
  tachometer's old position, since there's no engine speed to manage but
  there is a genuinely new continuous quantity (how hard the motor is
  working, in which direction) that a driver benefits from watching

Both directions demonstrate the actual skill this exercise tests: the
layout question was never "which of the two conventions do you prefer" —
it's "what does *this* driver, in *this* vehicle, need fastest," and
sometimes the honest answer is neither existing convention, which is worth
more credit than confidently picking one.

```notes
If a group defaulted to "speedometer, because that's normal," push them on
why — for a vehicle type with no legal top-speed anxiety (a slow-moving
delivery van) that default deserves real scrutiny, not automatic marks.
```
